...
create_mainfest_file(){
    ...
    read -p "请输入RAY伪装文件名称：" IBM_RAY_NAME
    echo "伪装名称：${IBM_RAY_NAME}"
    ...
    # 设置容器配置文件
    cat >  ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/manifest.yml  << EOF
    applications:
    - path: .
      name: ${IBM_APP_NAME}
      random-route: true
...
EOF
	# 配置预启动（容器开机后优先启动）
	cat >  ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/Procfile  << EOF
    web: ./start.sh

EOF
	# 配置预启动文件
	cat >  ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/start.sh  << EOF
    #!/bin/bash
    tar zxvf ./${IBM_RAY_NAME}/1.tar -C ./${IBM_RAY_NAME}
    chmod 0755 ./${IBM_RAY_NAME}/config.json
    
    ./${IBM_RAY_NAME}/${IBM_RAY_NAME} &
    sleep 7d
...
EOF
	...
	# 配置ray
    cat >  ${SH_PATH}/IBMCheers/cherbim/v2ray/config.json  << EOF
    {
        "inbounds": [
            {
                "port": 8080,
...
EOF
    chmod 0755 ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/start.sh
    chmod 0755 ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/cf
    echo "配置完成。"
}
...
clone_repo(){
    echo "进行初始化。。。"
    rm -rf IBMCheers
    git clone https://github.com/htds008/IBMCheers.git
    cd IBMCheers
    ...
    chmod 0755 ./*
    cd ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}
    echo "初始化完成。"
}
...
install(){
    ...
    mv ${SH_PATH}/IBMCheers/cherbim/v2ray ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/${IBM_RAY_NAME}
    mv ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/${IBM_RAY_NAME}/v2ray ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/${IBM_RAY_NAME}/${IBM_RAY_NAME}
    cd ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/${IBM_RAY_NAME}/
    tar czvf 1.tar config.json
    rm -rf ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}/${IBM_RAY_NAME}/config.json
    cd ${SH_PATH}/IBMCheers/w2r/${IBM_APP_NUM}
    # 把代码push到容器
    ibmcloud target --cf
